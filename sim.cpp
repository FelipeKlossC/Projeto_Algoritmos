...
Aluno calcularSituacao(Aluno a) { //pesquisar
    ...
    a.media = soma / MAX_NOTAS; //cálculo da média
...
int cadastrarAlunos(Aluno alunos[], int qtd) { //alunos é um parâmetro, um vetor da struct aluno
    ...
    for (int i = 0; i < n; i++) {
        ...
        do {
        	printf("Quantas notas esse aluno tera? (max %d)", MAX_NOTAS);
        	scanf("%d", &alunos[qtd].numNotas);
        	getchar();
        	if (alunos[qtd].numNotas < 1 || alunos[qtd].numNotas > MAX_NOTAS) {
        		printf("Quantia de notas inválida! Digite um valor de 1 a 20");
			}
		} while (alunos[qtd].numNotas < 1 || alunos[qtd].numNotas > MAX_NOTAS);

        for (int j = 0; j < alunos[qtd].notas; j++) {
            do {
                printf("Nota %d: ", j + 1);
